...
ax.fill_between(agg["n"], sem_lo, sem_hi,
                color=ROSE_WINE["rose"], alpha=0.25, linewidth=0)
ax.plot(agg["n"], agg["mean_hamming"], "-o",
        color=ROSE_WINE["deep_wine"], markersize=8, linewidth=2.4,
...
if len(nz_filt) >= 2:
    slope, intercept, r_value, _, _ = stats.linregress(
        np.log(nz_filt["n"]), np.log(nz_filt["mean_hamming"]),
    )
    ax.text(0.05, 0.06, f"fit slope: {slope:.2f}  ($R^2={r_value**2:.2f}$)",
...
ax.fill_between(agg["n"],
                1.0 - agg["p_err_hi"], 1.0 - agg["p_err_lo"],
                color=ROSE_WINE["rose"], alpha=0.25, linewidth=0)
ax.plot(agg["n"], 1.0 - agg["p_err"], "-o",
        color=ROSE_WINE["deep_wine"], markersize=8, linewidth=2.4)
ax.set_xscale("log")
ax.set_xlabel(r"sample size $n$")
